...
contract FuzzHarness {
    function chainCompressT2(uint256[2] calldata input, uint256 domainSep, uint256 n)
        external
        pure
        returns (uint256[] memory outs)
    {
        ...
        for (uint256 i = 0; i < n; i++) {
            uint256 out = Poseidon2T2_BN254.compress(st, domainSep);
            outs[i] = out;
            st[0] = out;
            st[1] = out;
        }
    ...
    function chainCompressUncheckedT2(uint256[2] calldata input, uint256 domainSep, uint256 n)
        external
        pure
        returns (uint256[] memory outs)
    {
        ...
        uint256[2] memory st = input;
        for (uint256 i = 0; i < n; i++) {
            uint256 out = Poseidon2T2_BN254.compressUnchecked(st, domainSep);
            outs[i] = out;
            st[0] = out;
            st[1] = out;
        }
    ...
    function chainCompressT3(uint256[3] calldata input, uint256 domainSep, uint256 n)
        external
        pure
        returns (uint256[] memory outs)
    {
        ...
        for (uint256 i = 0; i < n; i++) {
            uint256 out = Poseidon2T3_BN254.compress(st, domainSep);
            outs[i] = out;
            st[0] = out;
            st[1] = out;
            st[2] = out;
    ...
    function chainCompressUncheckedT3(uint256[3] calldata input, uint256 domainSep, uint256 n)
        external
        pure
        returns (uint256[] memory outs)
    {
        ...
        uint256[3] memory st = input;
        for (uint256 i = 0; i < n; i++) {
            uint256 out = Poseidon2T3_BN254.compressUnchecked(st, domainSep);
            outs[i] = out;
            st[0] = out;
            st[1] = out;
            st[2] = out;
